    ...
    <v-flex>
                    ...
                    <table id="facarticulo">
                      ...
                      <tbody>
                        <tr v-for="det in detalles" :key="det.iddetalleVenta">
                          ...
                          <td style="text-align:center;">{{ det.idarticuloNavigation.nombre }}</td>
                          <td style="text-align:center;">{{ det.precio }}</td>
                          <td style="text-align:center;">{{ det.descuento }}</td>
                          <td style="text-align:center;">{{ det.cantidad*det.precio-det.descuento }}</td>
                        </tr>
                      </tbody>
                      <tfoot>
                        <tr>
                        ...
                        <tr>
                          ...
                          <th style="text-align:right;">$ {{ totalImpuesto=(total*totalImpuesto)/(100+impuesto) }}</th>
                        </tr>
                        <tr>
                          <th></th>
                          <th></th>
      ...
      <v-data-table :headers="headers" :items="ventas" :search="search" class="elevation-1" v-if="verNuevo == 0">
        ...
        <template v-slot:item.actions="{ item }">
          ...
          <v-icon v-if="esAdministrador" small color="red accent-4" class="mr-2" @click="borrarRegistro(item)">
            delete
          </v-icon>
          <template v-if="item.estado == 'Aceptado'">
